# Engineering notes — the history behind CLAUDE.md

> Moved verbatim out of CLAUDE.md on 26 Sep 2026 so it no longer loads into every session.
> CLAUDE.md holds the operative rules; this file holds the incidents, audits and measurements
> that produced them. Search it (`grep -n`) when a rule needs its backstory. Figures here are
> point-in-time and are NOT checked by `verify:counts`.
