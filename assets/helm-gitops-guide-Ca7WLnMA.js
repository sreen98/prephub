const e=`# Helm & GitOps — Interview Guide
...
A single service needs a Deployment, Service, Ingress, ConfigMap, HPA (Horizontal Pod Autoscaler) and ServiceAccount. Multiply by three environments and twenty services and you have 360 YAML files differing only in replica counts, image tags and hostnames. Copy-paste diverges within a sprint.
...
Helm 3 removed **Tiller** (the in-cluster server component Helm 2 used, which held broad permissions and was a genuine security problem). Helm 3 is a **client-side** tool: it renders templates locally and talks to the API server with **your** credentials, so RBAC (role-based access control — Kubernetes' permission system) applies to you rather than to a privileged daemon. Release state is stored as Secrets in the release's namespace.

Charts can be hosted in **OCI registries** (\`oci://registry/charts/app\` — OCI is the Open Container Initiative format that container registries speak), which means the same registry and auth as your container images.
...
helm install --dry-run=server --debug mychart   # render + server-side validate
helm lint mychart
helm diff upgrade my-release mychart            # plugin: show what would change
\`\`\`

\`helm template\` is client-side only; \`--dry-run=server\` also validates against the API server, so it catches a bad \`apiVersion\` or a schema violation that \`template\` won't. Spell out \`=server\`: since Helm 3.13 a bare \`--dry-run\` means \`--dry-run=client\`, which is documented as making no cluster connection.
...
Note **CRDs are special** (custom resource definitions — the objects that add new resource kinds, like a cert-manager \`Certificate\`, to the Kubernetes API): Helm installs those in \`crds/\` but does **not** upgrade or delete them, deliberately, because deleting a CRD deletes every custom resource of that kind. CRD upgrades are a manual step.
...
Flux v2 is the other CNCF-graduated option (the Cloud Native Computing Foundation hosts Kubernetes itself; "graduated" is its top maturity tier), built as a set of composable controllers (source, kustomize, helm, notification, image automation) rather than one application with a UI.
...
Whichever you pick: Kubernetes Secrets are **base64, not encrypted**, so enable **encryption at rest** on etcd (the database where Kubernetes stores every object, Secrets included) and lock down RBAC regardless.
...
The key idea: the rollout **queries your metrics** ([Observability guide](/devops/observability-sre)) — here the success rate, an SLI (service level indicator: a number that stands for "is this working for users") — and **automatically aborts and rolls back** when the error rate or latency degrades — so a bad release is reverted in minutes without a human noticing. **Blue/green** is the alternative shape when you need instant, all-at-once cutover with a tested-but-dark environment.
