...
export default function ReviewPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    getAllQuestions()
      .then((qs) => { if (!cancelled) setAllQuestions(qs); })
      .catch((err: unknown) => {
        console.error('[prephub] review queue failed to load', err);
        if (!cancelled) setAllQuestions([]);   // shows the empty state, not a stuck spinner
      });
            ...
            >
              {!isFlipped ? (
                <div className="min-h-[250px] p-8 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm flex flex-col">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-4 text-center">Tap to reveal answer</p>
                </div>
              ) : (
                <div className="min-h-[250px] p-8 rounded-2xl border border-emerald-200 dark:border-emerald-800/50 bg-gradient-to-br from-emerald-50/50 to-teal-50/50 dark:from-emerald-950/20 dark:to-teal-950/20 shadow-sm flex flex-col">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-4 text-center">Tap to see question</p>
                </div>
              )}
            </motion.div>
          </AnimatePresence>
